...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
     ...
     */
    let buildings: string[] = ['food', 'fun', 'shop', 'sea'];

    /*
    // 升级建筑
    res = await api('user/BuildLvlUp', '_cfd_t,bizCode,ddwCostCoin,dwEnv,ptag,source,strBuildIndex,strZone', {ddwCostCoin: 317, strBuildIndex: 'shop'})
    console.log(res)
    if(res.iRet===0){
      console.log(`升级成功`)
    }*/
    ...
    // 任务1
    /*
    let tasks: any = await api('story/GetActTask', '_cfd_t,bizCode,dwEnv,ptag,source,strZone')
