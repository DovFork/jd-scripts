...
!(async () => {
  ...
  await requestAlgo();
  let res = {}, res2 = await getAuthorShareCode("https://example.com/")
  if (new Date().getHours() <= 3) res = await getAuthorShareCode(https://example.com/');
  if (!res2) res2 = await getAuthorShareCode('https://example.com/')
